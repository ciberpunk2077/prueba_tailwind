...
class Command(BaseCommand):
    help = 'Generar 300 muestras de polen nativas de Villahermosa, Tabasco'

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('Iniciando generación de 300 muestras de polen...')
        )
        ...
        def get_villahermosa_municipio():
            ...
            try:
                ...
                self.stdout.write(f"Municipio creado: {municipio.nombre}")
                return municipio

        def create_fake_image(nombre_especie):
            """Crear una imagen fake simple para la muestra de polen"""
            # Crear un archivo simple sin usar NamedTemporaryFile
            import tempfile
            import os
        ...
        for i in range(300):
            ...
            try:
                ...
                with open(imagen_path, 'rb') as img_file:
                    muestra.imagen.save(f"polen_{i+1}.txt", File(img_file), save=True)
        ...
        self.stdout.write(f"Familias creadas: {len(familias_creadas)}")
        self.stdout.write(f"Especies creadas: {len(especies_creadas)}")
